...
/*   Updated: 2023/04/22 23:12:47 by soohlee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_able_check(int keycode, t_mlx_data *mlx_db)
{
	...
	x = mlx_db->xy[0];
	y = mlx_db->xy[1];
	if (mlx_db->map[y][x] == 'D' && (keycode != 13 && keycode != 1))
		ft_end(mlx_db, mlx_db->row_len, 3);
	else if (keycode == 53 || (keycode == 13 && mlx_db->map[y - 1][x] == 'D'))
		ft_end(mlx_db, mlx_db->row_len, 3);
